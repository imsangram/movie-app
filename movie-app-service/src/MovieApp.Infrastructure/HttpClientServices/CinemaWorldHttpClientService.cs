...
{
    public class CinemaWorldHttpClientService(HttpClient httpClient, ILogger<CinemaWorldHttpClientService> logger, IMapper mapper) : ICinemaProviderService
    {
        ...
        {
            var response = await httpClient.GetAsync($"movie/cw{id}");
            if (response.IsSuccessStatusCode)
            {
